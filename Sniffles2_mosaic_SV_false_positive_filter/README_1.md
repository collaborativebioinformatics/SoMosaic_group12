...
```
.
├── sv_rf_filter.py                  inference: load a model, annotate a VCF (CLI + importable)
├── sv_fp_filter.ipynb               training, evaluation, and the analysis that produced the model
├── models/
│   └── sv_rf_filter_final.joblib    the trained bundle {model, feats, thresh, provenance}
├── docs/
│   └── feature_correlation.png      feature correlation matrix (below)
└── requirements.txt
```
